...
COLORS = {
    "hair": ["dark_brown", "black", "blonde", "ginger", "platinum", "pink"],
    "cloth": ["white", "blue", "green", "red", "yellow", "purple", "brown", "black"],
...
LICENCE_ORDER = ("CC0", "OGA-BY 3.0", "OGA-BY 3.0+", "CC-BY 4.0", "CC-BY 3.0",
                 "CC-BY-SA 4.0", "CC-BY-SA 3.0")

def license_of(d):
    """Pick the least restrictive licence the asset offers."""
    ls = {l.strip() for c in d.get("credits", []) for l in c.get("licenses", [])}
    for pref in LICENCE_ORDER:
        if pref in ls:
            return pref
    for l in ls:
        if "OGA-BY" in l or "CC-BY" in l:
            return l
    return None
