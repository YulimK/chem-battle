...
function paint(html) {
  // The syntax guide shows LaTeX source, so it must not be treated as math.
  // Its markup is parked in a data attribute, restored after KaTeX has run.
  const hints = [];
  html = html.replace(/<div class="hint">([\s\S]*?)<\/div>\s*(?=<)/g, (m, inner) => {
    hints.push(inner);
    return `<div class="hint" data-i="${hints.length - 1}"></div>`;
  });

  app.innerHTML = html;
  const fill = () => app.querySelectorAll('.hint[data-i]').forEach(el => {
    el.innerHTML = hints[Number(el.dataset.i)];
    el.removeAttribute('data-i');
  });

  if (!window.renderMathInElement) { fill(); return; }   // CDN still loading
  try {
...
function adoptWeekly(rows) {
  // The list carries counts only. Any bodies already loaded (by the editor or
  // by a student opening the week) are kept, otherwise saving would replace
  // the questions on screen with empty placeholders.
  const loaded = {};
  db.weekly.forEach(w => {
    if (w.questions.length && !w.questions.some(q => q === null)) loaded[w.week] = w.questions;
  });
  db.weekly = (rows || []).sort((a, b) => a.week - b.week).map(r => {
    const keep = loaded[r.week];
    return {
      week: r.week, title: r.title, status: r.status,
      completionReward: r.completion_reward, correctReward: r.correct_reward,
      questions: (keep && keep.length === (r.question_count || 0))
        ? keep
        : new Array(r.question_count || 0).fill(null)
    };
  });
}
  ...
  SB.channel('chem')
    ...
    .on('postgres_changes', { event: '*', schema: 'public', table: 'weekly' },
        async () => {
          ...
          if (!data) return;
          adoptWeekly(data);
          // If the editor is open and its bodies were dropped, pull them back
          // so the professor never sees an empty form after saving.
          const w = db.weekly.find(x => x.week === currentWeekly);
          if (page === 'weekedit' && w && w.questions.some(q => q === null)) {
            return window.editWeek(currentWeekly);
          }
          render();
        })
...
function weekly() {
  shell(`<div class="page">
    ${db.weekly.map(w => {
      ...
      const empty = !w.questions.length;
      if (r) { state = 'done'; label = `+${r.reward} <i class="g pot"></i>`; action = `openWeekly(${w.week})`; }
      else if (w.status === 'open' && empty) { state = 'closed'; label = '준비 중'; }
      ...
      return `<div class="weekrow ${state}" ${action ? `onclick="${action}"` : ''}>
        ...
        <p class="wstate">${r ? '✔ 완료'
          : (w.status === 'open' && empty) ? '문제 준비 중'
          : w.status === 'open' ? '진행 가능'
          : w.status === 'closed' ? '마감됨' : '잠김'}</p></div>
...
window.removeQuestion = qi => {
  const w = db.weekly.find(x => x.week === currentWeekly);
  if (!confirm(`문제 ${qi + 1}을 지울까요? 저장해야 반영됩니다.`)) return;
  w.questions.splice(qi, 1);
  render();
};
...
function weekedit() {
  const w = db.weekly.find(x => x.week === currentWeekly);
  const loading = w.questions.some(q => q === null);
  shell(`<div class="page">
    <button class="btn ghost" onclick="page='admin';render()">← Dashboard</button>
    ...
    <div class="panel">
      ...
      <div class="hint">
        <b>수식 쓰는 법</b>
        <p>달러 기호 사이에 씁니다. 화학식은 <code>\ce{ }</code> 안에 넣으면
           첨자가 자동으로 붙습니다.</p>
        <ul>
          <li><code>$\ce{H2SO4}$</code> → 황산</li>
          <li><code>$\ce{2H2 + O2 -> 2H2O}$</code> → 반응식</li>
          <li><code>$x^2$</code> → 위첨자 · <code>$a_1$</code> → 아래첨자</li>
          <li><code>$\frac{1}{2}$</code> → 분수</li>
        </ul>
      </div>
    ...
    ${loading ? '<div class="panel center empty">문제를 불러오는 중입니다...</div>'
      : (w.questions.length === 0
        ? '<div class="panel center empty">아직 문제가 없습니다. 아래에서 추가해 주세요.</div>'
        : w.questions.map((q, qi) => `<div class="panel">
      <div class="row"><b>문제 ${qi + 1}</b>
        <button class="btn inline red" onclick="removeQuestion(${qi})">삭제</button></div>

      <label class="flabel">문제</label>
      <textarea id="wq${qi}" class="input" rows="2" placeholder="예) 다음 중 화학식량이 가장 큰 것은?">${esc(q.q)}</textarea>

      ${[0, 1, 2].map(i => `
      <label class="flabel">선택지 ${i + 1}</label>
      <input id="wo${qi}_${i}" class="input" value="${esc(q.o[i] || '')}" placeholder="예) H₂O">`).join('')}

      <label class="flabel">정답</label>
      <select id="wa${qi}" class="input">${[0, 1, 2].map(i =>
        `<option value="${i}" ${(q.a ?? 0) === i ? 'selected' : ''}>선택지 ${i + 1}</option>`).join('')}</select>

      <label class="flabel">해설 <span class="small">(제출 후 학생에게 보입니다)</span></label>
      <textarea id="we${qi}" class="input" rows="2" placeholder="예) 분자량 비교: H₂O(18) < CH₄(16) < CO₂(44)">${esc(q.explanation)}</textarea>
    </div>`).join(''))}

    <button class="btn yellow" onclick="addQuestion()">+ 문제 추가</button>
    <button class="btn green" onclick="saveWeek()">저장</button>
    <p class="sub center">저장해야 학생 화면에 반영됩니다.</p>
  </div>`);
}
...
function admin() {
  ...
  shell(`<div class="page">
    ...
    <div class="panel">
      ...
      <div class="hint"><b>수식</b>
        <p>달러 기호 사이에 씁니다. 예: <code>$\ce{H2SO4}$</code>,
           <code>$x^2$</code>, <code>$\frac{1}{2}$</code></p></div>
      ${db.liveQueue.map(q => {
    ...
    <div class="panel">
      ...
      ${db.weekly.map(w => `<div class="mission">
        <div><b>${w.week}주차</b>${w.title && w.title !== `Week ${w.week}` ? ' · ' + esc(w.title) : ''}<br><span class="small">${w.status.toUpperCase()} · 문제 ${w.questions.length}개${
  w.status === 'open' && !w.questions.length ? ' · ⚠ 문제 없음' : ''}</span></div>
