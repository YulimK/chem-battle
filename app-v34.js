...
function missions() {
  // The number of live quizzes varies week to week, so the goal is however
  // many the professor has actually opened rather than a fixed three.
  const opened = db.liveQueue.filter(q => q.status !== 'ready');
  ...
  const list = [
    { key: 'attend', label: '출석하기', now: session.lastLogin === today() ? 1 : 0, goal: 1, reward: 5, auto: true }
  ];
  if (opened.length) {
    list.push({ key: 'live', label: 'Live Quiz 참여', now: answered, goal: opened.length, reward: 0, auto: true });
  }
  list.push({ key: 'weekly', label: 'Weekly Quiz 완료',
              now: Math.min(weeklyCount, currentWeek()), goal: currentWeek(), reward: 0, auto: true });
  return list;
}
