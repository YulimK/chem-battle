...
function adoptQuizzes(rows) {
  ...
  db.liveQueue = (rows || []).sort((a, b) => a.ord - b.ord).map(r => ({
    id: r.id, ord: r.ord, title: r.title, question: r.question,
...
async function pullShared() {
  const [q, w, m, r] = await Promise.all([
    SB.from('live_quiz_public').select('*'),
...
async function connect() {
  ...
  SB.channel('chem')
    ...
    .on('postgres_changes', { event: '*', schema: 'public', table: 'room' },
        p => { db.semesterComplete = !!(p.new?.data?.semesterComplete); render(); })
...
// Progress is the share of the 15 weekly quizzes a student has finished,
// so the bar advances one notch per week rather than tracking loose points.
const weeksDone = u => Object.keys(db.weeklyResults[u.nick] || {}).length;
const progressPct = u => Math.min(100, weeksDone(u) / TOTAL_WEEKS * 100);

function progressBar(u) {
  const n = weeksDone(u);
  return `<div class="xp">
    <div class="xpbar"><div class="xpfill" style="width:${progressPct(u)}%"></div></div>
...
function missions() {
  ...
  return [
    { key: 'attend', label: '출석하기', now: session.lastLogin === today() ? 1 : 0, goal: 1, reward: 20, auto: true },
    { key: 'live3', label: 'Live Quiz 참여 3회', now: Math.min(liveCount, 3), goal: 3, reward: 60 },
    { key: 'weekly1', label: 'Weekly Quiz 완료', now: Math.min(weeklyCount, 1), goal: 1, reward: 80 }
  ];
}
...
window.openWeekly = async w => {
  ...
  if (SB) return guard(async () => {
    ...
    if (r.done) {
      // Answers and explanations arrive only after grading.
      slot.questions = r.review.map((v, i) => ({ q: '', o: [], a: v.a, explanation: v.explanation }));
      db.weeklyResults[session.nick] = db.weeklyResults[session.nick] || {};
      db.weeklyResults[session.nick][w] = { correct: r.correct, reward: r.reward, answers: r.answers, review: r.review };
      const back = await rpc('get_weekly', { p_token: TOKEN, p_week: w });
      if (back.questions) slot.questions = back.questions.map((q, i) =>
        ({ ...q, a: r.review[i]?.a ?? 0, explanation: r.review[i]?.explanation || '' }));
    } else {
      slot.questions = (r.questions || []).map(q => ({ q: q.q, o: q.o, a: -1, explanation: '' }));
    }
  });
  render();
};
...
function my() {
  ...
  shell(`<div class="page">
    ...
    <div class="panel">
      ...
      <div class="mission"><span class="mtask">학기 진행</span><b>${weeksDone(session)} / ${TOTAL_WEEKS}주</b></div>
...
function ending() {
  ...
  shell(`<div class="page center ending">
    ...
    <div class="certificate">
      ...
      <p class="sub">${weeksDone(session)} / ${TOTAL_WEEKS}주 완료 · ${session.potions} <i class="g pot"></i></p>
    </div>
...
window.setWeek = async (w, st) => {
  const slot = db.weekly.find(x => x.week === w);
  slot.status = st;
  if (SB) return guard(() => rpc('admin_save_weekly', { p_token: TOKEN, p_week: {
    week: slot.week, title: slot.title, status: st,
    completion_reward: slot.completionReward, correct_reward: slot.correctReward,
    questions: slot.questions.filter(Boolean) } }));
  save(); render();
};
