...
function adoptQuizzes(rows) {
  ...
  const act = db.liveQueue.find(q => q.status === 'active' || q.status === 'ended');
  ...
  SB.channel('chem')
    .on('postgres_changes', { event: '*', schema: 'public', table: 'live_quiz' },
        async () => {
          ...
          if (data) { adoptQuizzes(data); tallyLoaded.clear(); render(); }
        })
    ...
    .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages' },
        p => {
          ...
          if (!db.messages.some(x => x.ts === Number(m.ts) && x.name === m.name)) {
            db.messages.push({ name: m.name, admin: m.admin, text: m.text, ts: Number(m.ts) });
            if (page === 'chat') render();
          }
        })
    .subscribe();
...
function admin() {
  ...
  shell(`<div class="page">
    ...
    <div class="panel">
      ...
      ${db.messages.length ? `<h3 style="margin-top:16px">최근 대화</h3>
        ${db.messages.slice(-8).map(m => `<div class="mission">
          <span class="mtask">${m.admin ? '<span class="badge admin">공지</span> ' : ''}<b>${esc(m.name)}</b> ${esc(m.text)}</span>
          <span class="small">${hhmm(m.ts)}</span>
        </div>`).join('')}` : ''}
    </div>
