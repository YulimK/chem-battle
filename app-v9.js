...
const APP_BUILD = 'v9-prof-wizard';
...
function stage(u, s = 4, temp = null) {
...
function login() {
  ...
  <div class="phone">
    <div class="page center login">
      ...
      <div class="field"><span class="fico">👤</span><input id="nick" class="input" value="${esc(r)}" placeholder="닉네임 (한글/영어)"></div>
      <div class="field"><span class="fico">🔒</span><input id="pin" class="input" type="password" inputmode="numeric" maxlength="4" placeholder="4자리 숫자 PIN"></div>
...
const NAV = [
  ['home', '🏠', '홈'], ['quiz', '⚔', '전투'], ['chat', '💬', '채팅'],
  ['shop', '🛒', '상점'], ['closet', '👕', '옷장'], ['my', '👤', '내 정보']
];

function shell(html, opts = {}) {
  const nav = session.isAdmin ? '' : `<nav class="nav">${NAV.map(x =>
    `<button class="${page === x[0] ? 'active' : ''}" onclick="go('${x[0]}')"><span class="ico">${x[1]}</span>${x[2]}</button>`).join('')}</nav>`;
  const left = opts.back ? `<button class="iconbtn" onclick="go('${opts.back}')">←</button>` : `<span class="potion">🧪 ${session.potions}</span>`;
  const title = opts.title ? `<b>${esc(opts.title)}</b>` : (opts.back ? '' : '<b>CHEM BATTLE</b>');
  const right = opts.back ? `<span class="potion">🧪 ${session.potions}</span>` : `<button class="iconbtn" onclick="logout()">↪</button>`;
  app.innerHTML = `<div class="phone"><div class="top">${left}${title}${right}</div>${html}${nav}</div>`;
}
    ...
    <div class="panel">
      ...
      ${ms.map(m => {
        ...
        return `<div class="mission">
          ...
          <span class="mmeta">${m.goal > 1 ? `<i class="prog">${m.now} / ${m.goal}</i>` : ''}<b>+${m.reward} 🧪</b></span>
        </div>`;
      }).join('')}
    </div>
    ${q && q.status === 'active' ? `<button class="btn green" onclick="go('live')">⚡ 진행 중인 LIVE QUIZ 참여</button>` : ''}
...
function quizHub() {
  shell(`<div class="page">
    <div class="panel"><h2>⚡ Live Quiz</h2><p class="sub">수업 중 교수님이 여는 실시간 퀴즈예요.</p><button class="btn green" onclick="go('live')">들어가기</button></div>
    <div class="panel"><h2>📖 Weekly Quiz</h2><p class="sub">주차별 복습 문제. 열린 주차부터 풀 수 있어요.</p><button class="btn yellow" onclick="go('weekly')">들어가기</button></div>
  </div>`, { title: '전투' });
}
  ...
  if (q.status === 'ended') {
    ...
    return shell(`<div class="page">
      <div class="panel center">
        ...
        <div class="rewardbox">
          <span>참여 보상 +${q.participationReward} 🧪</span><span class="divider"></span><span>정답 보상 +${q.correctReward} 🧪</span>
          <p class="gain">획득 포션 : +${reward} 🧪</p>
        </div>
      </div>
      <div class="panel">
...
function weekly() {
  shell(`<div class="page">
    ${db.weekly.map(w => {
      ...
      if (r) { state = 'done'; label = `+${r.reward} 🧪`; action = `openWeekly(${w.week})`; }
      else if (w.status === 'open') { state = 'open'; label = `최대 +${max} 🧪`; action = `openWeekly(${w.week})`; }
      ...
      return `<div class="weekrow ${state}" ${action ? `onclick="${action}"` : ''}>
        ...
        <span class="wmeta">${label}${state === 'locked' ? ' 🔒' : state === 'open' ? ' ›' : ''}</span>
      </div>`;
    }).join('')}
...
function weeklyrun() {
  ...
  shell(`<div class="page">
    ${review ? `<div class="reviewband">복습 · ${result.correct}/${w.questions.length} 정답 · 획득 +${result.reward} 🧪</div>` : ''}
  ...
  shell(`<div class="page">
    ...
    <div class="shopgrid">${list.map(it => {
      ...
      return `<button class="item ${tryOn[it.slot] === it.id ? 'sel' : ''}" onclick="tryOnItem('${it.id}')">
        ...
        <b>${esc(it.name)}</b>
        <span class="price">${owned ? '보유 ✓' : `${it.cost} 🧪`}</span>
      </button>`;
    }).join('')}</div>
  </div>`, { title: '상점' });
}
...
function closet() {
  ...
  shell(`<div class="page">
    <div class="panel center">
      ...
      <div class="chips">${slotTabs().filter(s => session.equipped[s[0]]).map(s => `<button class="chip" onclick="unequip('${s[0]}')">${s[1]} 벗기</button>`).join('')}</div>
    </div>
...
function my() {
  ...
  shell(`<div class="page">
    ...
    <div class="panel">
      <div class="mission"><span class="mtask">보유 포션</span><b>${session.potions} 🧪</b></div>
...
function ending() {
  ...
  shell(`<div class="page center">
    ...
    <div class="certificate">
      ...
      <p class="sub">Lv.${levelOf(session)} · ${session.potions} 🧪</p>
    </div>
  </div>`, { back: 'home', title: 'ENDING' });
}
...
loadAvatars().then(() => {
  ...
  if (prof) { dressProfessor(prof); save(); }
  render();
}).catch(e => {
